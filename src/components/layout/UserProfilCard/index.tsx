...
export default function UserProfilCard() {
  ...
  const fetchData = async () => {
    ...
    try {
      const userResponse = await fetch(`//api.kharisma-rental-center.my.id/user/${userId}`, {
        cache: 'no-cache',
      });
  ...
  const handleSubmitUpdate = async () => {
    const updateResponse = await fetch(`//api.kharisma-rental-center.my.id/users/${datas?.id}`, {
      method: 'PUT',
